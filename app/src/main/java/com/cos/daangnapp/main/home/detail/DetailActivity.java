...
import static com.cos.daangnapp.R.drawable.ic_favorite;
...
btnFavorite = findViewById(R.id.detaile_btn_favorite);
    ...
    public void inittListener(){
        btnFavorite.setOnClickListener(v -> {
            Log.d(TAG, "inittListener: 관심버튼 실행됨?");
            SharedPreferences pref = getSharedPreferences("pref", MODE_PRIVATE);
            int userId = pref.getInt("userId",0);
            Log.d(TAG, "inittListener: 유저Id 맞나"+userId);
            Log.d(TAG, "inittListener: 게시물 ID맞나"+postId);
            Call<CMRespDto<PostRespDto>> call = detailService.getlike(postId,userId);
            call.enqueue(new Callback<CMRespDto<PostRespDto>>() {
                @Override
                public void onResponse(Call<CMRespDto<PostRespDto>> call, Response<CMRespDto<PostRespDto>> response) {
                    Log.d(TAG, "onResponse: 관심 실행되고 왓어");
                    btnFavorite.setImageResource(ic_favorite);
                    notify();
                }
